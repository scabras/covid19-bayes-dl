...
p
pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/obsinc.pdf")
print(p)
dev.off()
```
...
p=ggplot(history,aes(x=1:nrow(history),y=val_loss))+
  ...
  ylab("Mean Squared Error (Incidence)")
p

pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/history.pdf")
p
dev.off()
```
...
p
pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/errors.pdf")
p
dev.off()
```
...
pmax=ggplot(subset(allres,day>as.Date("2021-01-01")),aes(x=day,y=cases,color=type))+
  facet_wrap(. ~ region)+geom_line()+
  theme(axis.text.x = element_text(angle = 90,size=rel(0.8)))+ ylab("Incidence")

p1
pmax

pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/preds1.pdf")
p1
dev.off()

pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/predsmax.pdf")
pmax
dev.off()
```
...
bayes.pred=function(allres.bayes,no.obs=FALSE){
  ...
  post.pred.summary=t(apply(cbind(size,p),1,function(x) qnbinom(c(0.005,0.5,0.995),size=x[1],prob=x[2])))
...
pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/bayes-preds1.pdf")
pmax
dev.off()
...
pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/bayes-preds-spain1.pdf")
pmax
dev.off()
...
If the model where trained upto `r last.observed-max_delay` these are the Bayesian predictions

```{r testsetoutput}
final.res.test=bayes.pred(allres.bayes.test,no.obs = TRUE)
toplot=subset(final.res.test,(day>last.observed-max_delay)&(day<last.observed))
pmax=ggplot(toplot,aes(x=day,y=observed))+geom_line()+
  geom_line(aes(x=day,y=median),colour="red")+
  geom_ribbon(aes(x=day,ymin=ici,ymax=sci),alpha = 0.5)+
              facet_wrap(. ~ region)+
...
pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/bayes-predsmaxtest.pdf")
pmax
dev.off()
```
...
```{r sc1preds}
last.days=(nrow(cvirus)-19):nrow(cvirus)
datsc1=cvirus
datsc1$MD[last.days]=datsc1$MD[min(last.days)]*(1.1)^((1:20))
est.lstm(datsc1)
...
pmax=ggplot(toplot,aes(x=day,y=median))+geom_line(colour="red")+
  geom_ribbon(aes(x=day,ymin=ici,ymax=sci),alpha = 0.5)+
  facet_wrap(. ~ region)+ylim(0,100)+
  theme(axis.text.x = element_text(angle = 90,size=rel(0.8)))+ ylab("Ratio of predicted incidence (scenario vs actual)")

pmax


pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/sc1.pdf")
print(pmax)
dev.off()
```
...
pmax=ggplot(toplot,aes(x=day,y=median))+geom_line(colour="red")+
  geom_ribbon(aes(x=day,ymin=ici,ymax=sci),alpha = 0.5)+
  facet_wrap(. ~ region)+ylim(0,50)+
  theme(axis.text.x = element_text(angle = 90,size=rel(0.8)))+ ylab("Ratio of predicted incidence (scenario vs actual)")

pmax

pdf(file="~/gd/Dropbox/Applicazioni/Overleaf/A Bayesian-DL model for estimating Covid-19 evolution in Spain/fig/sc2.pdf")
print(pmax)
dev.off()
```
