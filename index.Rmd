...
#conda_python(envname = "~/miniforge3/envs/r-reticulate/bin/python")
#conda_binary()
...
p
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/obsinc.pdf")
print(p)
dev.off()
```
...
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/history.pdf")
p
dev.off()
```
...
p
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/errors.pdf")
p
dev.off()
```
...
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/preds1.pdf")
p1
dev.off()

pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/predsmax.pdf")
pmax
dev.off()
```
...
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/bayes-preds1.pdf")
pmax
dev.off()
...
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/bayes-preds-spain1.pdf")
pmax
dev.off()
...
toplot=toplot[(toplot$day-min(toplot$day)+1)==toplot$delay,]
pmax=ggplot(toplot,aes(x=day,y=observed))+geom_line()+
  geom_line(aes(x=day,y=median),colour="red")+
  geom_ribbon(aes(x=day,ymin=ici,ymax=sci),alpha = 0.5)+
              facet_wrap(. ~ region)+
...
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/bayes-predsmaxtest.pdf")
pmax
dev.off()

ee=final.res.test
ee=ee[(ee$day>"2021-10-11")&((ee$day<="2021-10-18")),]
ee=ee[ee$day-as.Date("2021-10-11")==ee$delay,]

library(xtable)
tt=ee[c("day","region","ici","observed","median","sci")]
colnames(tt)=c("day","region","inf (0.5\\%)","obs.","pred (50\\%)","sup (99.5\\%)")
tt=melt(tt,id.vars = c("day","region"))
tt=dcast(tt,region+variable~day)
print(xtable(tt,digits = 0),include.rownames=FALSE)

rr=ee$observed-ee$median
sd(rr)
sqrt(mean((rr)^2))
summary(rr)
mean(ee$observed>ee$ici&ee$observed<ee$sci)
...
pmax=ggplot(toplot,aes(x=day,y=median))+geom_line(colour="red")+
  ...
  facet_wrap(. ~ region)+
  theme(axis.text.x = element_text(angle = 90,size=rel(0.8)))+
  ylab("Ratio of predicted incidence (scenario vs actual)")+ theme_bw()

pmax


pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/sc1.pdf")
print(pmax)
dev.off()
```
...
Suppose that the same hypothetical incidence in the first case were observed Canary Islands instead of Madrid:
...
pdf(file="~/gd/Articulos/elvirusdelacorona/mathematics/sc2.pdf")
print(pmax)
dev.off()
```
