...
We have `r nrow(cvirus)` days (in the range `r range(cvirus$day)`)  and `r ncol(cvirus)` variables which represent the cumulative incidence at 14 days per 100000 people along the regions.
...
```{r lstmtraintest}
est.lstm(cvirus,test.from="2021-01-01") # Model evaluation on Test set
...
If the model where trained on only 2020, this the output for year 2021.

```{r testsetoutput}
final.res.test=bayes.pred(allres.bayes.test)
toplot=subset(final.res.test,(day>as.Date("2021-01-01"))&(day<max(cvirus$day)))
pmax=ggplot(toplot,aes(x=day,y=observed))+geom_line()+
  geom_line(aes(x=day,y=median),colour="red")+
  geom_ribbon(aes(x=day,ymin=ici,ymax=sci),alpha = 0.5)+
              facet_wrap(. ~ region)+
...
pdf(file="../sort/fig/bayes-predsmaxtest.pdf")
pmax
dev.off()
```
